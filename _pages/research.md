...
#### **Concrete Topics of Interest**
1. Microarchitectural Weird Machines (uWMs): Recent work shows how the underlying microarchitecture can be used to hide malicious code execution by building weird circuits and weird programs that performs computation in the transient space. Currently, the applications of uWMs are limited due to its challenge to construct and limited interfaces. In this line of research, I wish to explore methods to extend its storage capability and lower the development complexities to widen uWM's applications. 
2. Microarchitectural (White/Black Box) Fuzzing: Recent research in microarchitectural security showed a trend of shifting from manually crafting exploits to automatic exploration. Fuzzing extensively test a hardware with diverse inputs to reveal patterns of exploits that were not permitted. In this line of research, I hope to develop fuzzing engines (pre-silicon & post-silicon) that can automatically test for known vulnerabilities and uncover novel exploitations.
